...
public class VentanaHamburguesas extends JFrame {
	...
	public VentanaHamburguesas(JFrame va, String nombre) {
		super();
		vActual=this;
		vAnterior = va;
	...
	private static void obtenerHamburguesas(String nombreCiudad) {
	    System.out.println("CARGANDO LOS BEBIDAS");
	    Restaurante.cargarProductosEnLista("ficheros/productos.csv");
	    String[] hamburguesasFijas = {"Diablo", "Pulled Pork", "Baking"};
	    
	    for (Producto p : Restaurante.getListaProductosFichero()) {
	        if (p.getTipoProducto().equals(TipoProducto.HAMBURGUESA)) {
	            if (Arrays.asList(hamburguesasFijas).contains(p.getNombre())) {
	                 crearBoton("imagenes/" + p.getImagen(), p.getNombre(), dividirDescripcionPorPalabras(p.getDescripcion()), p.getPrecio());
	            }   
	            if (nombreCiudad.equals("Bilbo") && p.getNombre().equals("Bilbo Burger")) {
	                crearBoton("imagenes/" + p.getImagen(), p.getNombre(), dividirDescripcionPorPalabras(p.getDescripcion()), p.getPrecio());
	            } else if (nombreCiudad.equals("Donosti") && p.getNombre().equals("Donosti Burger")) {
	                crearBoton("imagenes/" + p.getImagen(), p.getNombre(), dividirDescripcionPorPalabras(p.getDescripcion()), p.getPrecio());
	            } else if (nombreCiudad.equals("Gasteiz") && p.getNombre().equals("Gasteiz Burger")) {
	                crearBoton("imagenes/" + p.getImagen(), p.getNombre(), dividirDescripcionPorPalabras(p.getDescripcion()), p.getPrecio());
	            }
	        }
	    }
